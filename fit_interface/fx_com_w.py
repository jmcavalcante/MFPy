...
from fit_interface.py_from_ui.fx_com_d import Ui_MainWindow
...
class FitThread(QThread):
    ...
    def __init__(self, parent, folder, FZ0, p0,p_fit_FX0, lower_bounds, upper_bounds):
        super().__init__(parent)
        self.folder = folder
        self.FZ0 = FZ0
        self.p0 = p0
        self.p_fit_FX0 = p_fit_FX0
    ...
    def run(self):
        try:
            ...
            self.p_fit,_,_,self.FZ_data,self.FX_data,self.kappa_data,self.alpha_data,self.FX_initial,self.FX_fit = \
                mfpy.fit.FX_combined(self.folder,self.p_fit_FX0,FZ_nom=self.FZ0,initial_guess=self.p0.copy(),full_output=2,
...
class FX_com(QMainWindow):
    FX_com_task = pyqtSignal(mfpy.mf52_structure.MF52)
    def __init__(self,folder,tir_data,FZ0):
        ...
        self.p_fit_FX0 = [self.tir_data.LONGITUDINAL_COEFFICIENTS.PCX1,
        ...
        self.ui.table.setItem(self.index_cache.get('RBX1'),0,QTableWidgetItem(str(self.tir_data.LONGITUDINAL_COEFFICIENTS.RBX1)))
        self.ui.table.setItem(self.index_cache.get('RBX2'),0,QTableWidgetItem(str(self.tir_data.LONGITUDINAL_COEFFICIENTS.RBX2)))
        self.ui.table.setItem(self.index_cache.get('RCX1'),0,QTableWidgetItem(str(self.tir_data.LONGITUDINAL_COEFFICIENTS.RCX1)))
        self.ui.table.setItem(self.index_cache.get('REX1'),0,QTableWidgetItem(str(self.tir_data.LONGITUDINAL_COEFFICIENTS.REX1)))
        self.ui.table.setItem(self.index_cache.get('REX2'),0,QTableWidgetItem(str(self.tir_data.LONGITUDINAL_COEFFICIENTS.REX2)))
        self.ui.table.setItem(self.index_cache.get('RHX1'),0,QTableWidgetItem(str(self.tir_data.LONGITUDINAL_COEFFICIENTS.RHX1)))
        self.create_guess()
        ...
        self.fig = plt.figure()
        self.fig.suptitle('Longitudinal Force - combined fit')
        ...
        for i in range(len(self.FZ_data)):
            ...
            self.ax.scatter(self.kappa_data[i],self.FX_data[i],s=10,color=color,label=r'FZ {} N, $\alpha$ = {} rad'.format(self.FZ_data[i],self.alpha_data[i][0]))
        self.ax.grid()
    ...
    def create_guess(self):
        ...
        self.FZ_data,self.FX_data,self.kappa_data,self.alpha_data,self.FX_initial = mfpy.fit.FX_combined(self.folder,self.p_fit_FX0,FZ_nom=self.FZ0,initial_guess=self.p0.copy(),full_output='data_only')
    ...
    def start_fit(self):
        ...
        if self.fit_thread is None or not self.fit_thread.isRunning():
            self.fit_thread = FitThread(self, self.folder, self.FZ0, self.p0,self.p_fit_FX0, self.lower_bounds, self.upper_bounds)
    ...
    def edit_tir_data(self):
       self.tir_data.LONGITUDINAL_COEFFICIENTS.RBX1 = self.p_fit[0].round(7)
       self.tir_data.LONGITUDINAL_COEFFICIENTS.RBX2 = self.p_fit[1].round(7)
       self.tir_data.LONGITUDINAL_COEFFICIENTS.RCX1 = self.p_fit[2].round(7)
       self.tir_data.LONGITUDINAL_COEFFICIENTS.REX1 = self.p_fit[3].round(7)
       self.tir_data.LONGITUDINAL_COEFFICIENTS.REX2 = self.p_fit[4].round(7)
       self.tir_data.LONGITUDINAL_COEFFICIENTS.RHX1 = self.p_fit[5].round(7)
    ...
    def save_close(self):
        self.edit_tir_data()
        self.FX_com_task.emit(self.tir_data)
        self.close()
